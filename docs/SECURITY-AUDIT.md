...
审计日期：2026-09-05。范围：本次交付源码、现有五个 Linux/amd64 容器镜像证据及
新增 ARM64/1Panel Compose 发布设计。
本报告不能替代目标主机防火墙、DNS、证书、端口映射和后续依赖变化的重新审计。

## 发布结论

**AMD64 既有镜像通过；ARM64 发布必须等待双架构 CI 门禁。** 当前 AMD64 最终镜像与
交付源码未检出 Medium、High 或 Critical 问题。ARM64 镜像不能引用 AMD64 的既有扫描
结论；新流水线会分别扫描两种架构并生成 SBOM，全部成功后才发布多架构正式标签。
...
以上是本地 Docker 镜像 ID，不是 Docker Hub 清单摘要。重新构建会产生新 ID，必须
